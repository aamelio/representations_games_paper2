"""Resume, validate, track, and finalize the interrupted round-7 ratings.
...
LEGACY_OUTPUTS = HERE / "agent_outputs"
RATER_OUTPUTS = HERE / "similarity_rater_outputs"
CLASSIFICATION_RESUME = PACKETS / "classification_resume"
CLASSIFICATION_CHECKPOINTS = HERE / "classification_checkpoints"
PROGRESS = HERE / "round7_progress.json"
...
def initialize() -> None:
    ...
    (HERE / "resume_design.json").write_text(
...
def finalize() -> None:
    ...
    master = read_csv(HERE / "hp_master_working.csv")
    ...
    final.to_csv(HERE / "hp_round7_final.csv", index=False)
    final.to_excel(HERE / "hp_round7_final.xlsx", index=False)
    pd.DataFrame(agreement_rows).to_csv(
        HERE / "similarity_rater_agreement.csv", index=False
    )
    summary = {
        "rows": len(final),
    ...
    (HERE / "round7_final_manifest.json").write_text(
