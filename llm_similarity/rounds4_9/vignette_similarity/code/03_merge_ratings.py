#!/usr/bin/env python3
"""Validate and combine the three blinded final-vignette rating passes."""
...
INPUTS = {rater: RUN_DIR / f"relative_agent{rater}_ratings.csv" for rater in (1, 2, 3)}
PACKETS = {rater: RUN_DIR / "relative_packets" / f"rater_{rater}_tasks.json" for rater in INPUTS}
...
def main() -> None:
    ...
    combined.to_csv(RUN_DIR / "relative_ratings_3agents.csv", index=False)
    ...
    means.to_csv(RUN_DIR / "relative_ratings_3agent_means.csv", index=False)
    ...
    agreement.to_csv(RUN_DIR / "relative_rater_agreement.csv", index=False)
    ...
    (RUN_DIR / "relative_rating_protocol.json").write_text(json.dumps(protocol, indent=2) + "\n", encoding="utf-8")
