...
OUTPUT_DIR = RUN_DIR / "relative_packets"
RATER_SEEDS = {1: 84559, 2: 92173, 3: 104729}


def main() -> None:
    contexts = pd.read_csv(RUN_DIR / "anonymized_contexts.csv").set_index("t_id")["text"]
    vignettes = pd.read_csv(RUN_DIR / "anonymized_vignettes.csv").set_index("v_id")["text"]
    private_map = json.loads((RUN_DIR / "anonymization_map.json").read_text(encoding="utf-8"))
    ...
    for rater, seed in RATER_SEEDS.items():
        ...
        packet = {
            "rater": rater,
            "instructions_file": "../../../prompts/relative_similarity_prompt.md",
            "important": (
