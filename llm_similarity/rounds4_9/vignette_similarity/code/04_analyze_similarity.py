#!/usr/bin/env python3
"""Analyze the current rating run and form 95% t-intervals across model runs."""
...
OUTPUT_DIR = ROOT / "output" / "full_current_run"
GAME_ORDER = ["DG", "UG", "TG"]
...
def load_data() -> pd.DataFrame:
    ratings = pd.read_csv(RUN_DIR / "relative_ratings_3agents.csv")
    mapping = json.loads((RUN_DIR / "anonymization_map.json").read_text(encoding="utf-8"))
    contexts = (
...
def main() -> None:
    ...
    print(f"Wrote current-run estimates, model-run intervals, and figures to {OUTPUT_DIR}")
