#!/usr/bin/env python3
"""Build the manuscript outputs: neutralized DG ratings plus retained UG/TG ratings."""
...
ORIGINAL_RUN = ROOT / "rating_runs" / "original"
CURRENT_RUN = ROOT / "rating_runs" / "dg_c_neutral"
OUTPUT = ROOT / "output"


def load_analysis_module():
    path = Path(__file__).resolve().parent / "04_analyze_similarity.py"
    spec = importlib.util.spec_from_file_location("vignette_similarity_analysis", path)
...
def combine_by_target_game(original: pd.DataFrame, current: pd.DataFrame) -> pd.DataFrame:
    old = original.loc[original["target_game"].isin(["UG", "TG"])].copy()
    old["ratings_source"] = "round8_original"
    new = current.loc[current["target_game"] == "DG"].copy()
...
def main() -> None:
    ...
    original_data = load_round(ORIGINAL_RUN)
    current_data = load_round(CURRENT_RUN)

    r8_struct_agent, r8_struct, r8_struct_diff_agent, r8_struct_diff = analysis.structural_outputs(original_data)
    r9_struct_agent, r9_struct, r9_struct_diff_agent, r9_struct_diff = analysis.structural_outputs(current_data)
    r8_within_agent, r8_within, r8_within_diff_agent, r8_within_diff = analysis.within_outputs(original_data)
    r9_within_agent, r9_within, r9_within_diff_agent, r9_within_diff = analysis.within_outputs(current_data)

    r8_struct = recenter_structural_intervals(r8_struct, original_data)
    r9_struct = recenter_structural_intervals(r9_struct, current_data)
    ...
    (OUTPUT / "manuscript_protocol.json").write_text(
        json.dumps(protocol, indent=2) + "\n", encoding="utf-8"
    )
    print(f"Wrote the manuscript vignette-similarity outputs to {OUTPUT}")
