#!/usr/bin/env python3
"""Extract the revised DOCX vignettes and build blinded round-8 inputs."""
...
VIGNETTE_SOURCE = HERE / "input" / "joint_action_vignettes_revised.docx"
...
def main() -> None:
    ...
    write_csv(HERE / "anonymized_vignettes.csv", ["v_id", "text"], public_vignettes)
    write_csv(HERE / "anonymized_contexts.csv", ["t_id", "text"], public_contexts)
    write_csv(HERE / "revised_vignettes_extracted.csv", list(vignettes[0]), vignettes)
    mapping = {
        "seed": SEED,
        "source_hashes_sha256": {
            str(VIGNETTE_SOURCE.relative_to(HERE)): hashlib.sha256(VIGNETTE_SOURCE.read_bytes()).hexdigest(),
            str(CONTEXT_SOURCE.relative_to(HERE.parent.parent)): hashlib.sha256(CONTEXT_SOURCE.read_bytes()).hexdigest(),
        },
        "notes": {
            "rating_pool": "Every context is rated against all 22 revised vignettes in Stage 1.",
    ...
    (HERE / "anonymization_map.json").write_text(json.dumps(mapping, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
